...
module.exports = function(passport) {
    //serializing the user => store user.id into the session obj
    passport.serializeUser(function(user, done) {
        done(null, user.id);
    });
    
    //deserializing the user => retrieve user instance from user database by the user.id stored in the session obj
    ...
    passport.use('local-signup', new LocalStrategy({
        ...
        function(req, email, password, done) {
            ...
            if (password !== req.body.password_confirm) {
                return done(null, false, req.flash('signupMessage', 'Password doesn\'t match.'));}
            ...
            if (User.users.find((user) => user.email === email)) {
                ...
                return done(null, false, req.flash('signupMessage', 'Username or email has already been taken.'));
            } else { 
                ...
                const user =  User.createUser(req.body.firstname, req.body.lastname, password, email);
                console.log(User.users);
                return done(null, user, req.flash('signupMessage', 'User registered successfully.'));
            }
        }
    ));
};
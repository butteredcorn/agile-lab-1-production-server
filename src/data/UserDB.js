...
function User() {
    ...
    //for testing purpose: user1pw: testing, user2pw: password
    this.users = [
        { id: 1, firstname: 'john', lastname:'joe', password: 'roCXawE/GiokG41o4LMulHUjPW+DCUDkOomAl17dCX4=', email:'hommer@example.com' },
        { id: 2, firstname: 'ruihao', lastname:'li', password: 'XohImNooBHFR0OVvjcYpJ3NgPQ1qq73WKhHvch0VQtg=', email: 'Homer.lee.0508@gmail.com' }
    ];
    
    //fake user's history database
    this.historyFiles = {
        1: [{ date:'1/21/2020, 8:10:20 AM',inputTemp:'40c', outputTemp:'104f' }, { date:'1/21/2020, 8:11:20 AM', inputTemp:'122f', outputTemp:'50c' }],
        2: [{ date:'1/21/2020, 8:12:20 AM',inputTemp:'35c', outputTemp:'95f' }, { date:'1/21/2020, 9:10:20 AM', inputTemp:'100f', outputTemp:'38c' }]
   }
    ...
    this.findUserByEmail = ( email, password, fn ) => {
        this.users.forEach(user => {
            ...
            if ( user.email === email && user.password === this.passwordEncrypt(password)) return fn( null, user );})
        return fn(null, null);
    }
    ...
    this.insertIntoHistory = async (user_id, inputTemp, outputTemp) => {
        console.log(user_id);
        let history = {};
        history['date'] = new Date().toLocaleString('en-US');
        history['inputTemp'] = inputTemp;
        history['outputTemp'] = outputTemp;
        this.historyFiles[user_id.toString()].push(history);
